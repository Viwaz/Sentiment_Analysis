...
def test_endpoints():
    ...
    assert health_data["active_model_reference"] == "afriberta_small", "Expected active model to be afriberta_small"
    ...
    assert info_data["active_model_reference"] == "afriberta_small"
    assert "model_name" in info_data
    assert "model_version" in info_data
    assert "model_family" in info_data
...
if __name__ == "__main__":
    # Wait a second for server to settle
    time.sleep(1)
    test_endpoints()
